...
pub const CURRENT_BLOCK_HEIGHT: u64 = 951_451;
...
pub fn add_utxo(utxos: Vec<Utxo>, new_utxo: Utxo) -> Vec<Utxo> {
    ...
    let mut some_utxos = utxos;
    some_utxos.push(new_utxo);
    some_utxos
}
...
pub fn get_tx_status(tx_pool: &HashMap<String, String>, txid: &str) -> String {
    ...
    if tx_pool.contains_key(txid) {
        return tx_pool.get(txid).unwrap().to_owned();
    }
...
pub fn halving_schedule(blocks: &[u64]) -> HashMap<u64, u64> {
    ...
    for value in blocks {
        let halvings = value / 210_000;
        new_hash_map.insert(*value, base >> halvings);
    }
    new_hash_map
}
...
pub fn extract_tx_version(raw_tx_hex: &str) -> Result<u32, String> {
    ...
    let decoded_tx_hex = decode(raw_tx_hex).map_err(|_| "Hex decode error")?;
